...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-radian-parse-csv-detail.ps1

ruby -Ilib tests/test_domain_review.rb

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-radian-parse-csv-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-radian-parse-csv-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-radian_parse_csv_mark-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
